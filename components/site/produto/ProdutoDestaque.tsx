...
export default function ProdutosPage() {
  ...
  const carregarDados = async (alive = true) => {
    try {
      ...
      // ✅ LOG pra depurar
      console.log("✅ GET statuses:", rotas.admin.api.produtosStatus);
      console.log("✅ GET produtos:", rotas.admin.api.produtos);
      ...
      const statusesRaw = resolveApi<Status[]>(statusRes.data);
      const produtosRaw = resolveApi<any[]>(produtosRes.data);

      const statuses = Array.isArray(statusesRaw) ? statusesRaw : [];
      const listaProdutos = Array.isArray(produtosRaw) ? produtosRaw : [];
  ...
  const toggleDestaque = async (produto: Produto) => {
    try {
      if (produto.destaque && produto.id_destaque) {
        console.log("✅ DELETE destaque:", rotas.admin.api.destaqueRemover(produto.id_destaque));
        ...
        console.log("✅ POST destaque:", rotas.admin.api.destaquesCriar);
  ...
  const toggleCatalogo = async (produto: Produto) => {
    try {
      if (produto.catalogo === 1) {
        console.log("✅ PUT catalogo nao:", rotas.admin.api.catalogoNao(produto.id_produto));
        ...
        console.log("✅ PUT catalogo sim:", rotas.admin.api.catalogoSim(produto.id_produto));
  ...
  const excluirProduto = async (id: number) => {
    ...
    try {
      console.log("✅ DELETE produto:", rotas.admin.api.produtoRemover(id));
