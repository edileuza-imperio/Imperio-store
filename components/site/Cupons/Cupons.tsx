...
export default function Cupons() {
  ...
  useEffect(() => {
    const carregarCupons = async () => {
      ...
      try {
        // ✅ agora chama via rota centralizada
        const res = await api.get(rotas.cupons.ativos);

        // ✅ blindagem: se a API não vier no formato esperado, não quebra
        const raw = res.data?.dados ?? [];
        const lista = Array.isArray(raw) ? raw : [];

        const dados: Cupom[] = lista.map((c: any) => {
          const tipo = c.tipo_codigo ?? c.tipo ?? "percentual";
          const descontoNum = c.desconto ?? 0;

          const desconto =
            tipo === "frete"
              ? "FREE"
              : tipo === "valor"
              ? `R$ ${Number(descontoNum).toFixed(2).replace(".", ",")}`
              : `${descontoNum}%`;

          const expiracao =
            c.expiracao
              ? String(c.expiracao).split("-").reverse().join("/")
              : "Indefinido";
              ...
              <article className="cx-card">
                ...
                <div className="cx-head" style={{ background: corTipo(cupom.tipo) }}>
                  ...
                  <div className="cx-discount">
                    ...
                    {cupom.tipo !== "frete" && cupom.desconto.includes("%") && <small>OFF</small>}
                  </div>
                </div>
